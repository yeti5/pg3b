...
public class ConnectControllerDialog extends JDialog {
	...
	public ConnectControllerDialog (UI owner) {
		...
		if (!Util.isWindows) refreshButton.setVisible(false);
	}

	private void refresh () {
		controllerListModel.removeAllElements();
		for (XboxController controller : XboxController.getAll())
			if (controller.poll()) controllerListModel.addElement(controller);
	...
	private void initializeEvents () {
		...
		Util.enableWhenModelHasSelection(controllerList.getSelectionModel(), connectButton);
	}
	...
	private void initializeLayout () {
		...
		{
			JLabel label = new JLabel("<html>Please choose the Xbox controller for the connected device.");
			getContentPane().add(
				label,
